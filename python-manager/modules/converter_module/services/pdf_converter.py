    ...
    def convert_pdf_to_docx(pdf_data: io.BytesIO) -> io.BytesIO:
        ...
        try:
            ...
            try:
                ...
                docx_data.seek(0)
                try:
                    with zipfile.ZipFile(docx_data, 'r') as zf:
                        test = zf.testzip()
                        if test is not None:
                            raise ValueError(f"Corrupted DOCX: first bad file {test}")
                except zipfile.BadZipFile:
                    raise ValueError("Output is not a valid DOCX (not a zip file)")
                docx_data.seek(0)

                logger.info("✅ PDF to DOCX conversion successful and validated")
                return docx_data
