...
import {
	...
	formatFileSize,
	FileWithParts,
} from "@/lib/upload/utils/tus-file-utils";
...
export const useTusFileUpload = () => {
	...
	const persistQueue = useCallback((queue: QueuedFile[]) => {
		...
		localStorage.setItem("tusQueue", JSON.stringify(serializable));
	}, []);

	useEffect(() => {
		try {
			const raw = localStorage.getItem("tusQueue");
			if (!raw) return;
			const parsed: Omit<QueuedFile, "file">[] = JSON.parse(raw);
			// Restore as waiting if they were mid-flight
			const restored: QueuedFile[] = parsed.map((item) => ({
				...item,
				file: undefined,
				status:
	...
	useEffect(() => {
		persistQueue(fileQueue);
	}, [fileQueue, persistQueue]);

	// UNIFIED STATUS UPDATE - Simplified with single responsibility
	const updateFileStatus = useCallback(
		(fileId: string, updates: Partial<QueuedFile>) => {
			setFileQueue((prev) =>
				prev.map((file) =>
	...
	const handleFileSelection = useCallback(
		(files: FileList | null) => {
			...
			const uniqueFiles = fileArray.filter((newFile) => {
				// allow attaching to waiting items; skip true duplicates already attached
				const existing = fileQueue.find(
					(existingFile) =>
						existingFile.name === newFile.name &&
						existingFile.size === newFile.size
				);
				if (!existing) return true;
				// If we had a waiting item without file, attach instead of treating duplicate
				if (existing.status === "waiting" && !existing.file) {
					existing.file = newFile;
					existing.status = "pending";
					return false; // don't add new entry
				}
				return false;
			});
			...
			if (totalFiles > TUS_CLIENT_CONFIG.maxFileSelection) {
				const remainingSlots =
					TUS_CLIENT_CONFIG.maxFileSelection - currentFileCount;
				if (remainingSlots <= 0) {
					setMessage(
						`Maximum ${TUS_CLIENT_CONFIG.maxFileSelection} files allowed. Remove some files first.`
					);
					return;
				}
				...
				setMessage(
					`${previousMessage}Only ${remainingSlots} more files can be added (${TUS_CLIENT_CONFIG.maxFileSelection} max).`
				);
	...
	// Helper to resume prior uploads if they exist (tus-js-client fingerprinted)
	const startWithResume = useCallback(async (upload: tus.Upload) => {
	...
	const uploadFileComplete = useCallback(
		(queuedFile: QueuedFile): Promise<void> => {
			return new Promise((resolve, reject) => {
				const file = queuedFile.file;
				if (!file) {
					reject(
						new Error("File reference missing; please reselect to resume.")
					);
					return;
				}

				console.log(
					`Uploading complete file: ${file.name} (${formatFileSize(file.size)})`
				);
				...
				const upload = new tus.Upload(file, {
					...
					retryDelays: TUS_CLIENT_CONFIG.retryDelays,
					 metadata: {
					  filename: file.name,
					  filetype: file.type,
					  userId: TUS_CLIENT_CONFIG.userId,
					  stage: TUS_CLIENT_CONFIG.stage,
					  ...
					  jobId: TUS_CLIENT_CONFIG.jobId || TUS_CLIENT_CONFIG.destinationPath?.split("/")[1] || "default-job",
					 },
					onError: reject,
				...
				startWithResume(upload).catch(reject);
			});
		},
		[startWithResume, updateFileStatus]
	);
		...
		(queuedFile: QueuedFile): Promise<void> => {
			return new Promise((resolve, reject) => {
				...
				const updateProgress = () => {
					const totalBytesUploaded = partBytesUploaded.reduce(
						(sum, bytes) => sum + bytes,
						0
					);
	...
	const processQueueDynamic = useCallback(async () => {
		...
		while (true) {
			...
			const batch = findOptimalBatch(
				remainingFiles,
				TUS_CLIENT_CONFIG.maxStreamCount
			);
			...
			setMessage(
				`Uploading ${batch.length} files (${totalStreams} streams)...`
			);

			// Start all files in batch simultaneously
			const batchPromises = batch.map(async (fileInfo) => {
				const queuedFile = fileQueueRef.current.find(
					(f) => f.id === fileInfo.id
				);
	...
	// Reselect a waiting item to reattach File and resume
	const reselectFile = useCallback((fileId: string) => {
		...
		input.onchange = (e) => {
			...
			setFileQueue((prev) =>
				prev.map((f) => {
					...
					// Optional safeguard: ensure same name/size before resuming
					if (f.size !== selected.size || f.name !== selected.name) {
						return {
							...f,
							status: "error",
							error: "Please select the same file to resume.",
						};
					}
					return { ...f, file: selected, status: "pending", error: undefined };
				})
			);
		};
		input.click();
