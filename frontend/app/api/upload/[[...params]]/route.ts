...
async function handleUploadComplete(uploadId: string, uploadInfo: UploadInfo): Promise<boolean> {
  ...
  try {
    ...
    // Construct object key: remove leading slashes from meta.destinationPath if present
    const cleanDestPath = (meta.destinationPath || "").replace(/^\/+/, "");
    const objectKey = path.join(cleanDestPath, finalFilename);
    ...
    console.error("Failed to upload to MinIO:", err);
    // We don't fail the request here, but we should probably alert or retry
  }
