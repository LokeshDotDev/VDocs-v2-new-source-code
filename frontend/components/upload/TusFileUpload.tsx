...
type TusFileUploadProps = {
	jobId: string | null;
};

export default function TusFileUpload({ jobId }: TusFileUploadProps) {
	...
	// Clear file queue and localStorage when jobId changes
	React.useEffect(() => {
		setFileQueue([]);
		localStorage.removeItem("tusQueue");
	}, [jobId, setFileQueue]);
	...
	const handleDrop = (e: React.DragEvent) => {
		...
		if (!TUS_CLIENT_CONFIG.jobId) {
			TUS_CLIENT_CONFIG.jobId = jobId;
			TUS_CLIENT_CONFIG.destinationPath = `jobs/${jobId}/raw/`;
		}
				...
				{fileQueue.length > 0 && (
					<div className='flex flex-col gap-2 bg-neutral-950 border p-3 rounded-xl max-h-96 overflow-auto'>
						{fileQueue.map((file) => (
							<div
								key={file.id}
								className='flex items-center gap-3 py-3 px-4 border rounded-lg bg-neutral-900'>
									...
									{file.status === "waiting" && (
										<div className='flex flex-col gap-2'>
											...
											<p className='h-4 flex items-center justify-between gap-3 text-xs text-muted-foreground'>
												<span>
													{formatFileSize(file.uploadedBytes || 0)} of{" "}
													{formatFileSize(file.size)}
												</span>
										...
										<div className='flex flex-col gap-2'>
											...
											<p className='h-4 flex items-center justify-between gap-3 text-xs text-muted-foreground'>
												{file.uploadedBytes !== undefined ? (
													<span>
														{formatFileSize(file.uploadedBytes)} of{" "}
														{formatFileSize(file.size)}
													</span>
												) : (
