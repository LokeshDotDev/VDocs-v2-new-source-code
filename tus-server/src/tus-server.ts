...
export async function handleSingleFileUpload(
	...
	const jobId = (metadata.jobId as string) || uploadId; // Use jobId from metadata if present, else fallback to uploadId

	// Validate required fields for MinIO object key
	if (!userId || !jobId || !stage || !relativePath) {
		logger.error("Missing required metadata for MinIO object key", {
			userId,
			jobId,
			stage,
			relativePath,
		...
		throw new Error(`Missing required metadata for MinIO object key: userId=${userId}, jobId=${jobId}, stage=${stage}, relativePath=${relativePath}`);
	}
