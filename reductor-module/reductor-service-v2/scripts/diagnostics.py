...
def extract_text_nodes(docx_path: str):
    ...
    try:
        ...
        # Save problematic XML to temp file
        temp_path = None
        try:
            with tempfile.NamedTemporaryFile(delete=False, suffix="_diagnostics_failed.xml", mode="wb") as f:
                f.write(xml)
                temp_path = f.name
        except Exception as file_exc:
            temp_path = f"[Failed to save XML: {file_exc}]"
        print("[ERROR] XML parse failed in diagnostics.py extract_text_nodes:")
