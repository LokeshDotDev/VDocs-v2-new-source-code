...
router.get('/download-zip', async (req, res) => {
  try {
    ...
    logger.info({ fileKey }, '[download-zip] Request received');
    
    if (!fileKey || typeof fileKey !== 'string') {
      logger.warn({ fileKey }, '[download-zip] Missing or invalid fileKey');
      return res.status(400).json({ error: 'fileKey query parameter is required' });
    }

    // Verify file exists in MinIO
    try {
      logger.info({ fileKey }, '[download-zip] Verifying file exists in MinIO');
      await minioClient.statObject(config.MINIO_BUCKET, fileKey);
      logger.info({ fileKey }, '[download-zip] File verified in MinIO');
    } catch (statErr: any) {
      logger.error({ fileKey, statErr }, '[download-zip] File not found in MinIO');
    ...
    const filename = fileKey.split('/').pop() || 'export.zip';
    res.setHeader('Content-Type', 'application/zip');
    res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);

    // Stream the ZIP file
    try {
      logger.info({ fileKey }, '[download-zip] Starting stream');
      const stream = await minioClient.getObject(config.MINIO_BUCKET, fileKey);
      
      stream.on('error', (err: any) => {
        logger.error({ err, fileKey }, '[download-zip] Stream error');
        if (!res.headersSent) {
          res.status(500).json({ error: 'Failed to download ZIP', details: err.message });
        } else {
          res.end();
        }
      });
      ...
      stream.on('end', () => {
        logger.info({ fileKey }, '[download-zip] Download complete');
      });

      stream.pipe(res);
    } catch (streamErr: any) {
      logger.error({ streamErr, fileKey }, '[download-zip] Stream creation error');
      if (!res.headersSent) {
    ...
    logger.error({ error, msg: error.message }, '[download-zip] Uncaught error');
    if (!res.headersSent) {
      return res.status(500).json({ 
